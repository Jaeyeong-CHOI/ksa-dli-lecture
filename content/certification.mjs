...
export const certificationErrors = [
  ['07번의 셀 번호가 화면의 [숫자]와 달라요', '이 페이지의 셀 번호는 제공된 원본에서 설명·코드 셀을 위에서부터 센 위치입니다. JupyterLab 왼쪽 [숫자]는 실행 횟수라 다릅니다. 각 단계의 Part/Task 제목, 코드 첫 줄, 같은 셀 안의 코드를 함께 대조하세요. 이미 교체했다면 Get Certification 주석의 원본 셀 번호로 찾습니다. 새 셀은 추가하지 않습니다.'],
  ['07번에서 default_FAISS / vecstores / instruct_llm이 없다고 나와요', '셀 4의 모델 설정, 셀 34의 문서 준비, 셀 36의 Vector Store 생성, 셀 38의 통합이 차례로 끝났는지 확인합니다. 그 뒤 셀 40의 RAG TODO 셀을 실행하세요. 커널을 재시작했거나 셀 실행을 건너뛰었다면 셀 4부터 이 페이지의 순서대로 다시 실행합니다.'],
  ['07번 셀 4에서 ModuleNotFoundError 또는 모델 연결 오류가 나요', '개인 PC가 아니라 NVIDIA 강좌의 JupyterLab인지, Code 셀의 커널이 수업 Python 환경인지 확인합니다. 환경 시작 직후라면 서비스 준비가 끝날 때까지 기다린 뒤 07번 셀 4만 다시 실행하세요. 필요한 패키지는 수업 환경에 포함되어 있습니다. 계속 모듈이 없으면 작업을 저장하고 강좌 런처에서 환경을 다시 실행하거나 강사에게 오류 마지막 줄을 보여 주세요. 키를 채팅이나 이 사이트에 입력하지 마세요.'],
  ['07번 셀 34에서 arXiv 다운로드 오류가 나요', 'cached_papers/2210.03629v3.pdf가 있으면 네트워크 다운로드 없이 사용합니다. 캐시가 없고 외부 다운로드가 차단되었다면 논문 PDF(https://arxiv.org/pdf/2210.03629v3)를 브라우저에서 받은 뒤, JupyterLab 왼쪽의 cached_papers 폴더를 열고 Upload Files 버튼으로 2210.03629v3.pdf라는 이름으로 올립니다. 다시 07번 셀 34부터 실행하세요. HTML 오류 화면을 PDF로 이름만 바꾸면 안 됩니다.'],
  ['07번 셀 36의 진행 숫자가 멈추거나 중간에 실패했어요', '임베딩 모델이 문서 조각을 처리 중인지 먼저 기다려 봅니다. 실제 오류가 나면 연결 상태를 확인한 뒤 07번 셀 36을 다시 실행하세요. 이 셀은 vecstores를 처음부터 다시 만듭니다. 완료 후 통합 셀 38 → RAG 셀 40 → 저장 셀 44 → 확인 셀 46 순서로 진행하세요. 실패한 일부 결과를 저장하지 않습니다.'],
  ['07번을 재시작한 뒤 work_dir / chunks / docstore가 없다고 나와요', '커널을 다시 시작하면 07번에서 만든 변수도 사라집니다. 아직 저장 전이면 같은 07번 노트북에서 원본 셀 4 → 34 → 36 → 38 → 40 → 44 → 46 순서로 다시 실행하세요. 이미 07 셀 46 완료 후 인덱스 폴더와 압축 파일이 남아 있다면 08번 설정·불러오기 단계부터 이어갈 수 있습니다.'],
  ['docstore_index가 없거나 파일을 찾지 못해요', '처음 시작한 랩이면 이 페이지의 원본 셀 4 → 34 → 36 → 38 → 40 → 44 → 46을 먼저 실행하세요. 07번 셀 46에서 저장 후 검색 OK를 확인한 뒤 같은 폴더의 08번으로 넘어갑니다. 이미 저장했다면 07번 셀 4와 08번 불러오기에 표시된 작업 폴더가 같은지 확인하세요. 압축 안에는 docstore_index/index.faiss와 index.pkl이 있어야 합니다. 09번에서는 server_app.py 바로 옆에 이 폴더가 있어야 합니다. 빈 파일이나 빈 인덱스로 대신하면 검색·평가가 되지 않습니다.'],
