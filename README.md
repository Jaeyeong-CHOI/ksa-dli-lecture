...
챕터 첫머리에서 배울 내용을 확인한 뒤, **소주제 하나 → 관련 코드 → 개별 실습 → 핵심 정리**로 한 단계씩 이동합니다. 이전/다음 버튼과 챕터 목차로 순서를 고를 수 있고, 선택한 소주제 주소는 새로고침·뒤로 가기에서도 유지됩니다. 원본 셀 해설은 필요할 때 펼치며, 기존 셀 직접 링크는 해당 소주제와 코드를 자동으로 엽니다. Google Drive 바로가기는 제공하지 않고 원본 정보는 각 노트의 참고자료에 모읍니다.
