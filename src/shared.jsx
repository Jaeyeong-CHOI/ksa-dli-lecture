...
function Chat({open, close, prefill, status, initialMode = 'lecture', questionContext='', clearQuestionContext}) {
  ...
  function reset() {
    if (mode === 'notebook') clearQuestionContext?.();
  ...
  async function submit(e) {
    ...
    if (!draft || abort.current) return;
    const q = mode === 'notebook' && questionContext ? `참고할 위치: ${questionContext}\n\n${draft}` : draft;
    const history = messages.slice(-6).map(m => ({role: m.role, content: m.content}));
    const controller = new AbortController();
    abort.current = controller;
    pendingQuestion.current = {mode, question:draft};
