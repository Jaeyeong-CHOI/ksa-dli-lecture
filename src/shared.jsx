...
function Chat({open, close, prefill, status}) {
  const dialog = useRef(), input = useRef(), scroll = useRef(), abort = useRef();
  const [messages, setMessages] = useState([]), [value, setValue] = useState('');
  ...
  function reset() {
    ...
    setNotice('새 대화를 시작했어요. 이전 질문과 답변은 다음 요청에 포함되지 않습니다.');
