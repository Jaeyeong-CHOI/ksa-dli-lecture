...
function SearchNotes({open, close, includeAgentic=false}) {
  ...
  const practiceResults = includeAgentic && query.trim() ? agenticPractice.steps.filter(s => (s.title+s.goal+s.actions.join('')+(s.lesson?JSON.stringify(agenticConcepts[s.lesson]):'')).toLowerCase().includes(query.toLowerCase())) : [];
...
function App() {
  ...
  return <><a href="#main" className="skip-link">본문으로 건너뛰기</a>
    ...
    <SearchNotes open={search} close={() => setSearch(false)} includeAgentic={agentic}/><Chat questionContext={questionContext} clearQuestionContext={()=>setQuestionContext('')} open={chat} close={() => setChat(false)} prefill={prefill} status={status} initialMode={slides ? 'lecture' : 'notebook'}/>
  </>;
}
