...
function SearchNotes({open, close}) {
  ...
  const practiceResults = query.trim() ? agenticPractice.steps.filter(s => ('에이전틱 코딩 실습 '+s.group+s.title+s.goal+s.actions.join('')+(s.lesson?JSON.stringify(agenticConcepts[s.lesson]):'')).toLowerCase().includes(query.trim().toLowerCase())) : [];
