...
function AnnotatedCapture({item}){
 ...
 return <figure><button className="cert-capture-open" type="button" onClick={()=>dialog.current.showModal()} aria-label={item.alt+' · 화살표 포함 크게 보기'}>{picture("inline")}</button><figcaption>{item.caption}{legend()}<small>이미지를 누르면 화살표와 함께 크게 볼 수 있습니다.</small></figcaption><dialog ref={dialog} className="cert-capture-dialog" aria-label={item.alt+" 확대"}><button type="button" className="cert-capture-close" onClick={()=>dialog.current.close()}>닫기</button><div>{picture("dialog")}</div><p>{item.caption}</p>{legend()}</dialog></figure>;
}
