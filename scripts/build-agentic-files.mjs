...
let guide='# 에이전틱 코딩 실습\n\nk-skill 기존 Skill 활용 → Skill Creator로 공공데이터 API Skill 제작 → 화면 검증 → Vercel MCP 배포.\n\n웹 안내: https://ksa.dli-lecture.com/agentic-coding\n\n기본 ZIP은 READ_ME.md, PROMPTS.md, EXAMPLES.md 세 파일입니다. 실제 API 응답과 참고 화면은 웹에서 따로 열 수 있습니다. 예시를 받는 것과 Skill 설치·실행은 다릅니다.\n\n실행 환경: ChatGPT 데스크톱 Work와 Skills/MCP 지원 계정, Node.js/npx, Python 3, Vercel 계정. Codex 앱/CLI는 @ 대신 $로 Skill을 선택합니다. 웹 일반 채팅과 로컬 Skill 설정은 동일하지 않습니다.\n\n검수 범위: 선택 Skill 파일 설치·공개 피드 조회, 국가유산청 실제 목록/상세 호출, 로컬 참고 페이지 조작을 실행했습니다. ChatGPT 로그인 후 Skill 선택·OAuth·Vercel Preview 생성은 계정 환경에서 별도 확인이 필요합니다. 공식 안내 화면은 로그인된 실습 화면이 아닙니다.\n';
